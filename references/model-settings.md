...
官方 CLI 没有公开独立 negative prompt 参数，Turbo 官方配方又使用 `CFG 0.0`。因此默认不要输出独立负面提示词字段；将必要排除项压成正向描述末尾的一句短约束。

若用户明确使用的第三方节点或封装提供 negative conditioning 字段，可按该实现的实际语义输出，但必须明确这是第三方工作流能力，不是 Krea 2 官方通用配方。
